...
def parse_args():
    ...
    parser.add_argument(
        "--dim", type=int, help="Dimensionality of input data", default=3
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=1e-3,
        help="Initial learning rate of the model during training",
    )
...
def main(args):
    ...
    loggers = [
        aegnn.utils.loggers.LoggingLogger(None if args.debug else log_dir, name="debug")
    ]
    ...
    if not args.debug:
        wandb_logger = pl.loggers.WandbLogger(
            project=project, save_dir=log_dir, settings=log_settings
        )
        if args.log_gradients:
            wandb_logger.watch(
                model, log="gradients"
            )  # gradients plot every 100 training batches
        loggers.append(wandb_logger)
    logger = pl.loggers.LoggerCollection(loggers)

    checkpoint_path = os.path.join(
        log_dir, "checkpoints", args.dataset, args.task, experiment_name
    )
    callbacks = [
    ...
    if args.gpus is not None and len(args.gpus) > 1:
        ...
        )  # Use DDPPlugin for distributed training
