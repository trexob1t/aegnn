...
class DetectionModel(pl.LightningModule):
    ...
    def __init__(
        ...
        **model_kwargs,
        ...
        self.num_outputs_per_cell = (
            num_classes + num_bounding_boxes * 5
        )  # (x, y, width, height, confidence)
        num_outputs = (
            self.num_outputs_per_cell * self.cell_map_shape[0] * self.cell_map_shape[1]
        )  # detection grid

        # Define network architecture by name.
        model_input_shape = torch.tensor(img_shape + (dim,), device=self.device)
        self.model = model_by_name(network)(
            dataset, model_input_shape, num_outputs=num_outputs, **model_kwargs
        )
    ...
    ###############################################################################################
    def training_step(
        self, batch: torch_geometric.data.Batch, batch_idx: int
    ) -> torch.Tensor:
        ...
        logging.debug(
            f"Loss computation on batch with {len(gt_bb)} ground-truth bounding boxes"
        )
        loss, losses_dict, iou = self.loss(
            outputs, bounding_box=gt_bb, bbox_batch=gt_bb_batch
        )
        loss_logs = {
            f"Train/Loss-{name.capitalize()}": value
        ...
        train_accuracy = compute_detection_accuracy(
            detected_bbox, gt_y=batch.y.detach().cpu(), gt_batch=gt_batch
        )
        train_map = compute_map(
            detected_bbox, gt_bbox=gt_bb.detach().cpu(), gt_batch=gt_batch
        )
        ...
        for name, value in losses_dict.items():
            self.log(
                f"Train/Loss-{name.capitalize()}", value, on_step=True, on_epoch=True
            )
    ...
    def validation_step(
        ...
        with torch.no_grad():
            gt_batch = gt_bb_batch.detach().cpu()
            loss, _, iou = self.loss(
                outputs, bounding_box=gt_bb, bbox_batch=gt_bb_batch
            )
            val_accuracy = compute_detection_accuracy(
                detected_bbox, gt_y=batch.y.detach().cpu(), gt_batch=gt_batch
            )
            val_map = compute_map(
                detected_bbox, gt_bbox=gt_bb.detach().cpu(), gt_batch=gt_batch
        ...
        self.log(
            "Val/Accuracy", val_accuracy, on_step=True, on_epoch=True, sync_dist=True
        )
    ...
    def configure_optimizers(self):
        optimizer = torch.optim.Adam(
            self.parameters(), weight_decay=1e-4, **self.optimizer_kwargs
        )

        scheduler = StepLR(
            optimizer, step_size=20, gamma=0.1
        )  # Reduces LR by factor of 0.1 every 20 epochs
    ...
    # Parsing #####################################################################################
    ###############################################################################################
    def parse_output(self, model_output: torch.Tensor) -> Tuple[
        torch.Tensor,
        torch.Tensor,
        torch.Tensor,
        torch.Tensor,
        ...
        y_norm_rel = torch.clamp(
            model_output[..., nr_bbox : nr_bbox * 2], min=0
        )  # Center y
        w_norm_sqrt = torch.clamp(
            model_output[..., nr_bbox * 2 : nr_bbox * 3], min=0
        )  # Height
        h_norm_sqrt = torch.clamp(
            model_output[..., nr_bbox * 3 : nr_bbox * 4], min=0
        )  # Width
        y_confidence = torch.sigmoid(
            model_output[..., nr_bbox * 4 : nr_bbox * 5]
        )  # Object Confidence
    ...
    def parse_gt(
        ...
        gt_cell_corner_offset = torch.stack(
            [gt_cell_corner_offset_x, gt_cell_corner_offset_y], dim=-1
        )
    ...
    ###############################################################################################
    def detect(
        self, model_output: torch.Tensor, threshold: float = None
    ) -> torch.Tensor:
        """Computes the detections used in YOLO: https://arxiv.org/pdf/1506.02640.pdf"""
        cell_map_shape = torch.tensor(
            model_output.shape[1:3], device=model_output.device
        )
        input_shape = self.input_shape.to(model_output.device)
        cell_shape = input_shape / cell_map_shape
        x_norm_rel, y_norm_rel, w_norm_sqrt, h_norm_sqrt, pred_conf, pred_cls_conf = (
            self.parse_output(model_output)
        )
        ...
        bbox_top_left_corner = cell_top_left[None, :, :, None, :] + torch.stack(
            [x_rel, y_rel], dim=-1
        )
        ...
        detected_bbox_idx = torch.nonzero(torch.gt(pred_conf, threshold)).split(
            1, dim=-1
        )
        ...
        pred_cls_conf = pred_cls_conf[
            torch.arange(pred_cls.shape[0]), pred_cls.squeeze(-1)
        ]
        ...
        det_bbox = torch.cat(
            [
                batch_idx.float(),
                detected_top_left_corner[:, 0, None].float(),
                detected_top_left_corner[:, 1, None].float(),
                detected_w.float(),
                detected_h.float(),
                pred_cls.float(),
                pred_cls_conf[:, None].float(),
                pred_conf,
            ],
            dim=-1,
        )
    ...
    def detect_nms(
        self, model_outputs: torch.Tensor, threshold: float = 0.6, nms_iou: float = 0.6
    ):
    ...
    def loss(
        ...
        losses_dict = {
            key: torch.zeros(1, device=model_output.device) for key in loss_keys
        }
        for batch_i in bbox_batch.long().unique():
            ...
            (
                ...
                pred_conf,
                pred_cls,
            ) = self.parse_output(model_output[batch_i])
            out = self.parse_gt(gt_bbox_i, input_shape, cell_map_shape)
            (
                gt_cell_corner_offset_norm,
                gt_bbox_shape_norm_sqrt,
                gt_cell_x,
                gt_cell_y,
            ) = out
            ...
            pred_cell_offset_norm = pred_cell_offset_norm[
                gt_cell_x, gt_cell_y, responsible_pred_bbox_idx, :
            ]
            ...
            pred_cell_shape_norm_sqrt = pred_cell_shape_norm_sqrt[
                gt_cell_x, gt_cell_y, responsible_pred_bbox_idx, :
            ]
            ...
            losses_dict["confidence"] += self.__lambda_confidence * confidence_loss
            losses_dict["confidence_no_object"] += (
                self.__lambda_no_object * confidence_no_object_loss
            )
        ...
        iou_mean = (
            torch.cat(ious).mean()
            if len(ious) > 0
            else torch.ones(1, device=model_output.device)
        )
